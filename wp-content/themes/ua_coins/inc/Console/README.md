...
**Повні монети імпорт не чіпає.** Існуючу монету `wp nbu parse-souvenir` перезаписує лише поки вона
«очікує оновлення» — галочка «Оновлювати з НБУ» на екрані монети (`Sync\SyncStatus`): бракує аверсу/
реверсу, опису чи тиражу, або з випуску минуло менше 60 днів (НБУ докладає фото пізніше). Після
кожного імпорту прапорець перераховується і знімається сам, коли все стягнуто; адмін може поставити
його вручну, щоб монету перезаписало ще раз. Ручні правки повної монети лишаються. `--force` — оновити
всі. `short_title` пишеться лише при створенні монети й далі не оновлюється ніколи, навіть для
монет в оновленні чи з `--force`. Кожен запуск пишеться в `{prefix}coin_sync_runs`; звіт — Coins → Синхронізація НБУ.
Після першого деплою: `wp coins backfill-sync-status`.
