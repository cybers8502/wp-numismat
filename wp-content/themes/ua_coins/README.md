...
**Дизайнери розбираються з тексту НБУ** (`Catalog\DesignerCredits`): «аверс:/реверс:» відкидаються,
«адаптація дизайну» / «програмне моделювання» / «дизайн –» задають роль, одна людина — незалежно від
порядку слів і апострофа. Переприв'язати всі монети й прибрати зайвих дизайнерів:
`wp nbu parse-souvenir --pages=all --designers-only` (спершу з `--dry-run`).
...
│   ├── DesignerCredits.php          ← розбір «Художник:/Скульптор:» НБУ на людей і ролі
│   ├── DesignerRegistry.php         ← пошук/створення дизайнера за ключем особи
